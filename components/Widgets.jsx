...
const Widgets = ({ newsResults, randomUsersResults }) => {
  ...
  return (
    ...
    <div className="sticky top-0 text-gray-700 space-y-3 bg-gray-100 pt-2 rounded-xl w-[90%] xl:w-[75%]">
        ...
        {randomUsersResults.slice(0, randomUserNum).map((randomUser) => (
            <div key={randomUser.login.username} className="flex items-center px-4 py-2 cursor-pointer hover:bg-gray-200">
              <img className="rounded-full " width="40" src={randomUser.picture.thumbnail} alt="random User picture" />  
